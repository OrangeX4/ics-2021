...
static def_DHelper(IU) {
    decode_op_r(s, id_src1, s->isa.instr.iu.rs1, false);
    decode_op_i(s, id_src2, s->isa.instr.iu.imm11_0, false);
    decode_op_r(s, id_dest, s->isa.instr.iu.rd, true);
}
...
def_THelper(main) {
    ...
    def_INSTR_IDTAB("??????? ????? ????? ??? ????? 00100 11", I, compi);
    def_INSTR_IDTAB("??????? ????? ????? ??? ????? 11001 11", I, jmp);
    def_INSTR_IDTAB("??????? ????? ????? ??? ????? 11000 11", B, jmpb);
