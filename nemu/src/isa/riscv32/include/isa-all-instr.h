...
#define INSTR_LIST(f) f(auipc) f(mul) f(mulh) f(divu) f(div) f(remu) f(rem) \
            f(addi) f(add) f(andi) f(and) f(ori) f(or) f(xori) f(xor) f(sub) \
            ...
            f(csrrs) f(csrrw) f(ecall) f(mret)
