...
往 `pcb[0]` 和 `pcb[1]` 中分别载入了 hello 和 pal 的内容.

在调用 `context_uload()` 函数载入程序过程中, 又执行了很多子过程.

首先通过 `protect(&pcb->as)` 初始化了程序对应的页目录, 并且将内核区的页目录也复制了一份过来, 为后续给程序加入页表项, 切换不同程序采用不同页目录的策略打好基础.

随后, 开辟 8 个新页作为用户栈的物理空间, 并使用 `map()` 函数将栈区的虚拟页和物理页一一映射起来, 将程序的执行参数 `argc`, `argv` 和 `envp` 给复制入用户栈中.

最后, 通过 `loader()` 函数加载程序的代码主体和相应的数据内容, 并且将加载好的数据, 也使用 `map()` 函数将虚拟页和物理页一一映射起来. 这里还有一个小细节, 就是将 `.bss` 节后续的位置作为 `pcb->max_brk` 载入.

至此, 我们就给 hello 和 pal 在 "可能会产生重叠" 的虚拟地址上, 分配了物理上不同的用户栈和代码数据空间.

**第二, 程序执行阶段**

在用户程序执行的过程中, 使用的地址实际上都是虚拟地址, 需要先进行转换成为物理地址, 才能数据进行数据的处理.

虚拟地址转换为物理地址的过程深入到了 NEMU 中, 关键在于 `isa_mmu_translate()` 这个函数中.
...
我们通过 `get_satp()` 获取到当前程序对应的页目录的地址, 进一步获取到页表地址, 最后通过页表项和地址的最后 12 位偏移值查询到了真实的物理地址.

程序执行阶段还有一个很关键的地方: 虽然其他数据的地址范围不变, 我们一定能保证页表项有效. 但是堆区一直是动态增长的, 我们并不能保证堆区增长之后依然在有效列表中.

所以我们需要在 `brk()` 系统调用的时候, 动态地使用 `mm_brk(uintptr_t brk)` 给新地址分配页表.

如果 `brk` 增长的范围超过了 `max_brk`, 并且进入了一个新页, 我们就使用 `map()` 函数对新地址进行映射.

通过这种方式, 我们就实现了 hello 和 pal 的并行的底层基础: 即使虚拟地址相同, 物理地址也不一样. 进而保证了不同程序数据的互不冲突.

**第三, 程序切换阶段**

在程序 hello 和 pal 相互切换时, 我们要执行一系列的过程.

首先是保存程序上下文. 在 `trap.S` 对应的代码里, 我们将对应的三十多个寄存器一一保存起来, 并且我们还需要对栈区小心地进行操作, 通过 `mscratch` 这个间接寄存器和上下文中的 `c->np`, 识别出用户栈到内核栈, 内核栈到用户栈之间的转换过程, 并做好对栈区的处理.

在 Nanos-lite 系统中, 我们使用 `schedule(Context *prev)` 函数进行 pcb 的切换, 更换到不同的程序上下文中, 也就实现了 hello 和 pal 的动态更换.

切换到新的程序上下文时, 我们还需要使用 `__am_switch(c)` 也就是 `set_satp(c->pdir)` 来实现不同程序不同页目录的切换.

同样关键的是, 到底什么才会发生程序的切换?

在 Nanos-lite 中, 我们是通过 yield 系统调用和时钟中断实现的, 我们将重点放在时间中断上.

硬件 NEMU 每隔 10ms 就会触发一次 `timer_intr()`, 然后调用 `dev_raise_intr()` 函数, 进而给 `cpu.INTR` 设置为高电平, 标志着可以触发时钟中断了. 

然后 `cpu_exec()` 若是检测到了 INTR 为高电平, 还有一些其他时钟中断相关的条件 (例如是否是开中断), 就会调用 `isa_raise_intr` 函数, 触发一个时钟中断, 进入到异常或中断处理环节, 然后就是我们熟悉的保存上下文和切换用户程序了.

通过这一系列的硬件 NEMU, 操作系统 Nanos-lite 之间的相互配合, 我们就成果将 hello 和 pal 以一种分时多任务的模式运行起来了!
...
我们注意到第一条语句 `char *p = "abc"` 中左侧是一个指针, 说明字符串 "abc" 并不位于栈区, 在在程序编译和链接之后, 字符串 "abc" 实际上是被放置于只读数据区 `.rodata` 节中.

我们执行 `p[0] = 'A'` 往这个字符串内写入值, 就相当于往 `.rodata` 节对应的内存区域写入值. 但是这时候编译器和链接器还不会报错, 因为它们并不能确定指针 `p` 就一定是指向了 `.rodata` 节.

对于运行时环境, `.rodata` 节在加载到内存时, 被加载入了只读代码段中, 操作系统也初始化了相应页表项的内容. 只读代码段的段表项设定了访问权限为只读, 对应页表项中也被设置了访问权限位为只读.

我们执行 `p[0] = 'A'`, 相当于对只读代码段中的内容进行写入操作, 这会触发访问权限保护, 实际访问操作和访问权限不符, 发生了存储保护错.

随后硬件检测出这个错误之后, 就进入异常响应阶段, 经过操作系统一系列处理后, 发现这是一个不可恢复的异常, 于是就报了段错误.

**验证猜想:**

执行 `readelf -p .rodata ./test` 命令, 输出 `.rodata` 节里面的数据:      
